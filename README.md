...
- Bilet kodları (`TKT-...`) `crypto.randomUUID()` ile üretilir — giriş yapmış olmak bileti hesaba "bağlamıyor", sadece satın alabilmek için önkoşul: sistem hâlâ kağıt bir bilet kadar güvenli, kodu bilen/QR'ı okutan biri check-in yapabilir (bu, gerçek e-biletlerin de genel çalışma prensibidir).
- **Misafir girişi gerçek Supabase Auth e-posta+şifre akışıdır** (`signUp`/`signInWithPassword`) — bilet almaya başlamadan (bir koltuğa tıklayıp modal açmadan) önce zorunlu, ama etkinlik/koltuk gezinmesi girişsiz kalıyor. Supabase panelinde **Authentication > Sign In / Providers > Email > "Confirm email"** kapalı tutulmalı — yoksa hesap oluşturduktan sonra bir onay maili beklemeden giriş yapılamaz (ki bu da Supabase'in ücretsiz e-posta gönderim kotasına bağımlı hale getirir, bkz. Emails > SMTP Settings notu).
