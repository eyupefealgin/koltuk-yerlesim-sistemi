...
1. `supabase-setup.sql` dosyasındaki SQL'in **tamamını** Supabase projenin **SQL Editor**'ünde çalıştır — `events`/`event_sales`/`seat_holds` tablolarını, `purchase_seat`/`reserve_seat`/`release_seat_hold`/`redeem_discount_code`/`cancel_ticket` fonksiyonlarını oluşturur. Script baştan sona tekrar tekrar çalıştırılabilir (idempotent), daha önce kısmen çalıştırdıysan sorun olmaz
